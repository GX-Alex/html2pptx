...
- Text is inside unsupported external icon fonts or web fonts that did not load.
...
The wrapper inlines a small offline Font Awesome Free solid subset before DOM extraction. Supported `<i class="fas fa-..."></i>` icons become SVG paths and are converted as editable geometry.

If icons are still missing:

- Check whether the icon is from Font Awesome Pro, Brands, Regular, or an unsupported Free icon not included in the subset.
- Check `sources/*.html2pptx-preprocessed.html` when using `--keep-workdir`; supported icons should appear as `<svg class="fa-inline-svg ...">`.
- Add the missing icon path to `scripts/fontawesome_subset.py`, or replace that icon with inline SVG in the source HTML.
